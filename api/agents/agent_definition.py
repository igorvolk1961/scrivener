...
try:
    ...
    # Для Python < 3.11 используем typing_extensions
...
class PromptsConfig(BaseModel, extra="allow"):
    ...
    user_info: dict[str, Any] | None = Field(
        default=None,
        description="User information for template replacement (e.g., userPost)",
    )
    queryRefinementCount: int = Field(
        default=3,
        description="Quantity of automatic refinements of user query, including original query as first element",
    )
