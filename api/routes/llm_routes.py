...
async def generate_response(
    ...
    try:
        ...
        cfx_client = CfxClient(base_referer_url, jsessionid)
        context = {}
        context["userInfo"] = cfx_client.get_current_user_info()
        if request.irv_id:
            context["irvInfo"] = cfx_client.get_irv_info(request.irv_id)
        else:
            context["irvInfo"] = {}
        chat_messages, irv_exists = load_chat_history(cfx_client, request.chat_history_irv_id)
...
async def manage_rag_files(
    ...
    try:
        ...
        # Извлечение контекста для работы с КФО
        base_referer_url, jsessionid = extract_callback_info(http_request)
        cfx_client = CfxClient(base_referer_url, jsessionid)
        
        # Инициализация RAG сервиса
        rag_service = QdrantService()
        
        if request.action == "add":
            result = rag_service.add_files_to_rag(request, cfx_client)
