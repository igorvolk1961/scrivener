"""
Загрузка и сохранение истории чата в КФО (файл chat_history.json при версии ИО).
"""
...
def load_chat_history(cfx_client: Any, chat_history_irv_id: Optional[str]) -> tuple[Optional[List[dict]], bool]:
    """
    ...
    try:
        ...
        irv = cfx_client.get_irv(chat_history_irv_id)
        ...
        raw_files = cfx_client.get_irv_files(chat_history_irv_id)
        ...
        content = cfx_client.get_irv_file_content(file_obj)
        if content is None:
            return (None, True)
...
def save_chat_history(
    ...
    *,
    ...
    """
    Сохраняет историю чата в КФО: либо создаёт новый ИО с файлом chat_history.json в папке
...
def _save_chat_history_new(
    ...
    *,
    ...
    if not irv_id or not str(irv_id).strip():
        ...
        return None
    try:
        current_irv = cfx_client.get_irv(irv_id)
        ...
        if not parent_id or not nau_id:
            ...
            return None
        raw_children = cfx_client.get_folder_children(parent_id)
        ...
        if dialogs_folder:
            ...
            created = cfx_client.create_folder(DIALOGS_FOLDER_NAME, parent_id, description=folder_description)
        ...
        create_result = cfx_client.create_ir(
            irv_name=title,
        ...
        new_irv = cfx_client.get_irv(new_irv_id, with_files=True)
        ...
        if not file_obj:
            ...
            return None
        cfx_client.post_irv_file_content(file_obj, body_str)
...
def _save_chat_history_update(
    ...
    *,
    ...
    """
    try:
        current_irv = cfx_client.get_irv(chat_history_irv_id)
        if not isinstance(current_irv, dict):
...
def save_result_file(
    ...
    try:
        ...
        irv_info = cfx_client.get_irv(result_irv_id, with_files=True)
        ...
        cfx_client.post_irv_file_content(file_obj, content)
