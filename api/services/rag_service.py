...
class QdrantService:
    """Сервис для работы с Qdrant и управления файлами в RAG-системе."""
    ...
    def add_files_to_rag(self, request: RAGRequest, cfx_client: CfxClient) -> Dict[str, Any]:
        """
        Добавление файлов ИО в векторную БД.
        
        Args:
            request: Запрос с параметрами
            cfx_client: Клиент для работы с КФО
        ...
        try:
            ...
            files_to_process = self._get_files_to_process(cfx_client, request.irv_id)
    ...
    def _get_files_to_process(self, cfx_client: CfxClient, irv_id: str) -> List[Dict[str, Any]]:
        """Получение и фильтрация файлов для обработки."""
        # Получение списка файлов
        raw_files = cfx_client.get_irv_files(irv_id)
    ...
    def _get_cached_config(self):
        ...
        if QdrantService._config_cache is None:
            from utils.config import get_config
            QdrantService._config_cache = get_config()
        return QdrantService._config_cache
    ...
    def _get_cached_embedding(self, embed_api_key: Optional[str] = None, embed_url: Optional[str] = None, embed_model_name: Optional[str] = None, embed_batch_size: Optional[int] = None):
        ...
        if cache_key not in QdrantService._embedding_cache:
    ...
    def _get_cached_vector_store(self, vdb_url: str, collection_name: str, vector_size: int, timeout: int, api_key: str = None):
        ...
        if cache_key not in QdrantService._vector_store_cache:
            from rag.vector_store import QdrantVectorStoreManager
        ...
        return QdrantService._vector_store_cache[cache_key]
    ...
    def _process_file(
        ...
        try:
            ...
            file_content_response = cfx_client.get_irv_file_content(file_data)
    ...
    def _extract_file_content(self, file_content_response: Any, file_name: str) -> bytes:
        """Извлечение содержимого файла из ответа КФО."""
    ...
    def _create_nodes_from_chunks(
        ...
        # Обработка обычных чанков (включая таблицы, которые могут быть в chunks с метаданными)
        for idx, chunk_data in enumerate(chunker_result.get("chunks", [])):
            ...
            node = TextNode(
                ...
                id_=f"{irv_id}_{irvf_id}_{chunk_type}_{idx}"
            )
            
            # Распределяем чанки по соответствующим спискам
            if chunk_type == "table":
