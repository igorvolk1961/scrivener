...
# КРИТИЧЕСКИ ВАЖНО: Настраиваем логирование ПЕРВЫМ ДЕЛОМ,
# ДО импорта любых модулей, которые могут использовать httpx/httpcore.
# Это необходимо, чтобы объекты Trace в httpcore создавались с правильным уровнем логирования.
from utils.logging import setup_logging
from utils.config import get_config

# Загружаем конфигурацию и настраиваем логирование ДО всех остальных импортов
_config = get_config()
_log_level = _config.get("logging", {}).get("level", "INFO")
...
def create_app() -> FastAPI:
    ...
    """
    # Загружаем конфигурацию (логирование уже настроено на уровне модуля)
    config = get_config()
