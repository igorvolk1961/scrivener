...
class DocumentRetriever:
    ...
    def __init__(
        ...
        use_qdrant_builtin_reranker: bool = False
    ):
        """
        ...
        Args:
            ...
            reranker: Опциональный внешний реранкер для переупорядочивания результатов
            use_qdrant_builtin_reranker: Использовать встроенный реранкер Qdrant
        """
        ...
        self.use_qdrant_builtin_reranker = use_qdrant_builtin_reranker
        
        reranker_status = "disabled"
        if use_qdrant_builtin_reranker:
            reranker_status = "qdrant_builtin"
        elif reranker:
            reranker_status = "external"
    ...
    def retrieve(
        ...
        if self.use_qdrant_builtin_reranker and results:
            logger.debug(f"Применение встроенного реранкера Qdrant к {len(results)} результатам")
            results = self._apply_qdrant_reranker(query, results, top_k, filter_metadata)
            logger.debug(f"После реранкинга Qdrant: {len(results)} результатов")
        elif self.reranker and results:
            logger.debug(f"Применение внешнего реранкера к {len(results)} результатам")
        ...
        logger.info(f"=" * 80)
        logger.info(f"Результаты поиска и реранкинга для запроса: '{query}'")
        logger.info(f"=" * 80)
        logger.info(f"Всего найдено результатов: {len(results)}")
        logger.info(f"Возвращено топ-{top_k}: {len(final_results)}")
        logger.info("")
        ...
        for i, result in enumerate(final_results, 1):
            ...
            logger.info(f"{'─' * 80}")
            logger.info(f"Результат [{i}/{len(final_results)}]")
            logger.info(f"{'─' * 80}")
            logger.info(f"ID чанка: {result_id}")
            logger.info(f"Score (релевантность): {score:.6f}")
            ...
            if metadata:
                ...
                logger.info("  (метаданные отсутствуют)")
            logger.info("")
            logger.info("Полный текст фрагмента:")
            logger.info("-" * 80)
            # Выводим полный текст с сохранением форматирования
            if text:
    ...
    def _apply_qdrant_reranker(
        ...
        """
        Применение встроенного реранкера Qdrant.
        
        Выполняет поиск только среди переданных документов, позволяя Qdrant
        автоматически применять встроенный реранкинг для улучшения результатов.
        
        Args:
            query: Текст запроса
            documents: Список документов для реранкинга (поиск будет выполнен только среди них)
            top_k: Количество топ-результатов
            filter_metadata: Фильтр по метаданным
        
        Returns:
            Отсортированные результаты после реранкинга
        """
        if not documents:
            return []
        ...
        try:
            ...
            query_embedding = self.embedding._get_query_embedding(query)
            if not query_embedding:
                logger.warning("Не удалось получить эмбеддинг для реранкинга Qdrant")
            ...
            if not document_ids:
                logger.warning("Не найдено ID в документах для реранкинга")
                return documents[:top_k]
            
            # Создаем фильтр по ID найденных документов
            # Это ограничит поиск только этими документами
            id_filter_condition = HasIdCondition(has_id=document_ids)
            
            # Подготавливаем фильтр по метаданным (если есть)
            metadata_filter = self._prepare_filter(filter_metadata)
            
            # Объединяем фильтры: фильтр по метаданным + фильтр по ID
            filter_conditions = [id_filter_condition]
            ...
            filter_dict = self._filter_to_dict(combined_filter)
            
            # Используем обычный поиск с фильтром по ID найденных документов
            # Qdrant автоматически применяет встроенный реранкинг при поиске
            search_url = f"{self.vector_store_manager.url}/collections/{self.vector_store_manager.collection_name}/points/search"
            
            # Лимит равен количеству найденных документов (не больше)
            search_limit = len(document_ids)
            ...
            response = httpx.post(search_url, json=search_payload, timeout=30)
            response.raise_for_status()
            search_results = response.json().get("result", [])
            
            reranked_results = self._parse_search_results(search_results)
            logger.debug(
                f"Встроенный реранкер Qdrant вернул {len(reranked_results)} результатов "
                f"(из {len(document_ids)} найденных документов, возвращено топ-{top_k})"
            )
            
            return reranked_results[:top_k]
            
        except Exception as e:
            logger.error(f"Ошибка при применении встроенного реранкера Qdrant: {e}", exc_info=True)
    ...
    def get_context(
        ...
        if not chunks:
            logger.info(f"Контекст для запроса '{query[:100]}...': не найдено чанков")
            return ""
        ...
        # Логируем подробную информацию о сформированном контексте
        logger.info(f"=" * 80)
        logger.info(f"Контекст для запроса: '{query}'")
        logger.info(f"=" * 80)
        logger.info(f"Количество чанков в контексте: {len(chunks)}")
        logger.info(f"Общая длина контекста (с разделителями): {len(context)} символов")
        logger.info(f"Общая длина текста чанков: {total_text_length} символов")
        logger.info(f"Длина разделителей: {len(context) - total_text_length} символов")
        logger.info("")
        
        # Логируем подробную информацию о каждом чанке в контексте
        for i, chunk in enumerate(chunks, 1):
            ...
            logger.info(f"{'─' * 80}")
            logger.info(f"Чанк [{i}/{len(chunks)}] в контексте")
            logger.info(f"{'─' * 80}")
            logger.info(f"ID чанка: {chunk_id}")
            logger.info(f"Score (релевантность): {score:.6f}")
        ...
        logger.info(f"Полный контекст (объединенный текст всех чанков):")
        logger.info("=" * 80)
        logger.info(context)
        logger.info("=" * 80)
        logger.info("")
