...
class DocumentIndexer:
    ...
    def index_document(
        ...
        # Создание узлов из всех типов чанков: обычных, оглавления и таблиц
        nodes = self._create_nodes_from_chunks(
            chunker_result.get("chunks", []),
            doc_metadata
        )
        
        # Создание узлов из чанков оглавления
        toc_nodes = self._create_nodes_from_chunks(
            chunker_result.get("toc_chunks", []),
            doc_metadata,
            chunk_type="toc"
        )
        
        # Создание узлов из чанков таблиц
        table_nodes = self._create_nodes_from_chunks(
            chunker_result.get("table_chunks", []),
            doc_metadata,
            chunk_type="table"
        )
        
        # Объединяем все узлы для индексации
        all_nodes = nodes + toc_nodes + table_nodes
        
        # Генерация эмбеддингов и сохранение в векторное хранилище
        indexed_count = self._index_nodes(all_nodes)
        
        result = {
            "document_id": document_id,
            "document_path": str(document_path),
            "chunks_processed": len(chunker_result.get("chunks", [])),
            "toc_chunks_processed": len(chunker_result.get("toc_chunks", [])),
            "table_chunks_processed": len(chunker_result.get("table_chunks", [])),
        ...
        logger.info(
            ...
            f"{indexed_count} узлов (обычных: {len(nodes)}, оглавление: {len(toc_nodes)}, таблицы: {len(table_nodes)})"
        )
    ...
    def _create_nodes_from_chunks(
        ...
        Args:
            ...
            chunk_type: Тип чанков ("text", "toc", "table")
        ...
        for idx, chunk_data in enumerate(chunks):
            ...
            node_metadata = {
                ...
                **chunk_metadata,
                ...
                "chunk_id": f"{document_metadata['document_id']}_{id_prefix}_{idx}"
            }
        ...
        logger.debug(f"Создано {len(nodes)} узлов типа '{chunk_type}' из {len(chunks)} чанков")
    ...
    def _index_nodes(self, nodes: List[TextNode]) -> int:
        ...
        try:
            ...
            for idx, (node, embedding) in enumerate(zip(nodes, embeddings)):
                # Отладочное логирование для первых узлов
                if idx < 3:
                    logger.info(f"Узел {idx} перед сохранением в Qdrant:")
                    logger.info(f"  Метаданные узла: {node.metadata}")
                    logger.info(f"  Ключи в метаданных: {list(node.metadata.keys())}")
