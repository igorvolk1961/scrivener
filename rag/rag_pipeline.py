    ...
    def _init_components(self) -> None:
        ...
        if reranker_config.get("enabled", False):
            ...
            if use_qdrant_builtin_reranker:
                logger.info("Используется встроенный реранкер Qdrant")
            else:
                try:
                    ...
                    logger.info("Внешний реранкер инициализирован")
                except Exception as e:
                    logger.warning(f"Не удалось инициализировать внешний реранкер: {e}")
                    reranker = None
